...
const features = [
  {
    icon: <Users className="w-8 h-8 text-rhiz-teal" />, 
    title: 'Contact Management',
    desc: 'Organize, enrich, and search your relationships in real time.'
  },
  {
    icon: <Target className="w-8 h-8 text-rhiz-gold" />,
    title: 'Goal Tracking',
    desc: 'Set, track, and achieve goals with AI-powered contact matching.'
  },
  {
    icon: <Share2 className="w-8 h-8 text-rhiz-purple" />,
    ...
    <section id="features" className="py-16 bg-white">
      <div className="max-w-5xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((f, i) => (
            <div key={i} className="flex flex-col items-center p-8 rounded-xl shadow-md bg-rhiz-gray-50 hover:shadow-xl transition">
              {f.icon}
